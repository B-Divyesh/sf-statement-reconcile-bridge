# Landing copy audit

| Sentence                                                                         | Words | Result                         |
| -------------------------------------------------------------------------------- | ----: | ------------------------------ |
| Private file-to-file reconciliation                                              |     3 | pass                           |
| Reconcile your statement with your ledger                                        |     6 | pass                           |
| For people with a private budget file who need to check a monthly bank download. |    15 | pass                           |
| Try it with sample data                                                          |     6 | pass                           |
| See suggested matches right away.                                                |     5 | pass                           |
| Start with your files                                                            |     4 | pass                           |
| Files stay in this browser.                                                      |     5 | pass                           |
| Works offline after the first visit.                                             |     6 | pass                           |
| Free for the full reconciliation job.                                            |     6 | pass                           |
| Move through a monthly statement                                                 |     5 | pass                           |
| Add a bank CSV, OFX, or QIF and your ledger CSV.                                 |    12 | pass                           |
| See one-to-one matches with the reason for each score.                           |    10 | listed `match-score-reasons`   |
| Hand approved rows and an audit report back to your ledger.                      |    12 | pass                           |
| Local cleanup rules                                                              |     3 | pass                           |
| Save recurring payee wording                                                     |     4 | pass                           |
| Keep bank wording and ledger wording aligned before you review suggestions.      |    11 | pass                           |
| Set a cleanup rule                                                               |     4 | pass                           |
| No bank login.                                                                   |     3 | pass                           |
| No budget advice.                                                                |     3 | pass                           |
| This is a bridge between files you already control.                              |    10 | pass                           |
| Matches are suggestions.                                                         |     3 | pass                           |
| You make the final call.                                                         |     5 | pass                           |
| Reconcile statement files without changing your ledger.                          |     7 | listed `input-files-unchanged` |
| Original illustration generated for this product.                                |     6 | pass                           |

No landing-page sentence is over 22 words and no banned term appears.

## Terminology

| Concept                 | Product word  |
| ----------------------- | ------------- |
| Bank export             | statement     |
| Existing private record | ledger        |
| Paired record           | match         |
| Suggested pairing       | suggestion    |
| Final reviewed output   | reviewed rows |
| Record of actions       | audit report  |
